...
### 4. 智能文件存储中心 (Aegis File Hub)
内置高度抽象的文件管理体系，超越简单的文件上传：
- **策略模式架构**：一键切换 `LOCAL`、`ALIYUN_OSS`、`MINIO` 等存储方式，业务代码零改动。
- **自动目录归档**：本地存储原生支持按 `yyyy/MM` 自动分库分表式存储，解决单文件夹文件过多的性能问题。
- **智能资源回收**：删除文件时自动递归清理空的父文件夹，确保文件系统时刻保持整洁。
- **环境迁移无忧**：数据库仅存储相对路径，URL 动态构建。当服务从测试环境迁移到生产环境（域名变更）时，所有历史文件链接自动失效并更新为新域名。

### 5. 极速异步日志 (Non-blocking Logging)
...
├── common/             # 基础实体类 (Snowflake ID)、统一响应结果集
├── config/             # 全局配置 (Sa-Token, WebMvc, 文件映射)
├── controller/         # 业务接口 (含公共文件管理接口)
├── db/                 # DatabaseInitService 数据库自动演进引擎
├── entity/             # 业务实体 (基于 Code-First 注解)
├── enums/              # 业务枚举与存储策略类型
├── mapper/             # MyBatis-Plus Mapper 接口
├── service/            # 业务逻辑 (含 FileContextService 编排层)
│   └── storage/        # 存储策略实现 (本地、阿里云等)
└── utils/              # 辅助工具 (IP 获取、项目重命名工具)
```
...
## 愿景
**Aegis-Boot** 致力于解决传统框架过于臃肿、性能低下、安全配置繁琐的问题。我们通过最先进 of Java 技术栈，为您打造一个更现代、更安全、更丝滑的开发底座。
